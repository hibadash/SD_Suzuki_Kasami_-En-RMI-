...
public class NodeProduction extends UnicastRemoteObject implements INode {
    private int id;
    private int N;
    private int[] RN;
    private Token token;
    ...
    public NodeProduction(int id, int N, IStock stock) throws RemoteException {
        this.id = id;
        this.N = N;
        this.stock = stock;
        this.RN = new int[N];
        this.others = new HashMap<>();
        this.hasToken = (id == 0); // premier noeud avec jeton
        if (hasToken) token = new Token(N);
    }
    ...
    public void enterCS() throws RemoteException {
        System.out.println("[PRODUCTION] Section critique: ajout d’un carton...");
        stock.addCarton();
        exitCS();
    }
    ...
    public void exitCS() throws RemoteException {
        ...
        if (!token.Q.isEmpty()) {
            ...
            hasToken = false;
            System.out.println("[PRODUCTION] Jeton envoyé à " + next);
        }
    }
